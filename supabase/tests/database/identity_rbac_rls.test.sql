...
select is((select count(*) from public.permissions), 17::bigint, 'permission contract contains 17 codes');
select is((select count(*) from public.role_permissions rp join public.roles r on r.id = rp.role_id where r.code = 'owner'), 17::bigint, 'owner has every permission');
select is((select count(*) from public.role_permissions rp join public.roles r on r.id = rp.role_id where r.code = 'admin'), 12::bigint, 'admin has the documented 12 permissions');
select is((select count(*) from public.role_permissions rp join public.roles r on r.id = rp.role_id where r.code = 'finance'), 9::bigint, 'finance has the documented 9 permissions');
select is((select count(*) from public.role_permissions rp join public.roles r on r.id = rp.role_id where r.code = 'staff'), 4::bigint, 'staff has the documented 4 permissions');
