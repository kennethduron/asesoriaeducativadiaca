-- Temporary, narrowly scoped bridge for the one-time F8 Legacy task import.
-- A following migration removes it immediately after reconciliation.
...
begin
  ...
  if legacy_assignee_label not in ('Equipo DIACA', 'Equipo legal', 'Equipo académico')
    ...
    or legacy_done is distinct from false
  ...
  insert into public.tasks (
    ...
    created_by, priority, status, due_at, created_at, updated_at,
    migration_metadata
  ) values (
    legacy_task_id,
    btrim(legacy_title),
